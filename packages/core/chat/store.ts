...
export function createChatStore(options: ChatStoreOptions) {
  ...
  const store = create<ChatState>((set, get) => ({
    ...
    setChatSize: (width, height) => {
      logger.debug("setChatSize", { width, height });
      storage.setItem(CHAT_WIDTH_KEY, String(width));
      storage.setItem(CHAT_HEIGHT_KEY, String(height));
      set({ chatWidth: width, chatHeight: height, isExpanded: false });
    },
    setExpanded: (expanded) => {
      logger.info("setExpanded", { to: expanded });
      if (expanded) storage.setItem(wsKey(CHAT_EXPANDED_KEY), "true");
      else storage.removeItem(wsKey(CHAT_EXPANDED_KEY));
