...
// Personal inbox events are sent only to the target user via SendToUser.
// All other events are broadcast to the workspace room.
func registerListeners(bus *events.Bus, hub *realtime.Hub) {
	...
	// member:added — also send to the added user so they discover the workspace.
	// Pass excludeWorkspace so clients already in the target room don't receive
	// the event twice.
